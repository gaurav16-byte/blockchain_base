...
def to_peer(uid, user, public_key, ip, port):
    conn = sqlite3.connect('peer_table.db')
    cursor = conn.cursor()
    cursor.execute('INSERT INTO NODES VALUES(?, ?, ?, ?, ?)', [uid, user, public_key, ip, port])
    conn.commit()
    conn.close()
...
def store_private_in_pem(private_key_pem):
    with open(os.environ['HOME'] + '/.confid/private_key.pem', 'wb') as private_key_file:
...
def read_private():
    with open(os.environ['HOME'] + '/.confid/private_key.pem', 'rb') as private_key_file:
